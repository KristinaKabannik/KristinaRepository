...
def find_cars_by_criteria(car_data, search_criteria):
   cars_results = [
     (car, characteristics) for car, characteristics in car_data.items()
     if characteristics[1] >= search_criteria[0]
        and characteristics[2] >= search_criteria[1]
        and characteristics[4] <= search_criteria[2]
   ]

   cars_results_sorted = sorted(cars_results, key=lambda x: x[1][4])
   return cars_results_sorted[:5]

cars_results = find_cars_by_criteria(car_data, search_criteria)
print("First five cars matching the search criteria (2017, 1.6, 36000) and sorted by price (asc):")
for car, characteristics in cars_results:
  print(f"{car}: {characteristics}")